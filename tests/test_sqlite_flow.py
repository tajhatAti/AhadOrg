...
print("[8] RunSpace degrades gracefully with no runner configured")
r = client.post("/api/execute", headers=auth, json={"language": "python", "code": "print(1)"})
check(r.status_code == 503 and "runner" in r.text.lower(), "execute -> 503 w/ clear detail (no runner)")
r = client.get("/api/jobs", headers=auth)
check(r.status_code in (200, 503), "jobs list never 500s without runner")
